...
<h2 style="color:blACK" align="center" ><font face="Algerian">租借紀錄</font></h2>
...
%>	
	 <td width="514" valign="top"><table border="0" cellspacing="0" cellpadding="0" width="100%">
       <tr>
        <td><h3 style="color:blue;">最近租借紀錄</h3></td>
		<td><a href="detail.jsp?book_id=<%=book_id%>&book_name=<%=book_name%>" class="btn iframe"> <button class="ui-hidden">所有租借紀錄</button></a></td>
      </tr>
	  <tr>
      ...
      <tr>
        <td colspan="2"><p>借桌遊日期：<%=borrow_date%></p></td>
      </tr>
      <tr>
        <td colspan="2"><table border="0" cellspacing="1" cellpadding="0" width="100%">
          <tr>
...
%>			
 ...
 <%
	...
	if(even_rent == 0){
%>
<h1 align="center" color="blue">您目前租借的桌遊都已歸還~~~</h1>
<%}%>
</table>
</div>
</body>
...
function getreserve(i,sql_status,rent_borrow_id,book_name,book_id,book_price,member_id,member_name,member_email){
	...
	if(sql_status == "insert"){
		checkbook =  confirm("確定租借桌遊:" + book_name + "?");
	}else if(sql_status == "delete"){
		checkbook =  confirm("確定取消這桌遊的預約:" + book_name + "?");
	}
		  ...
		  $.ajax({
            ...
            success: function(data,textStatus,jqXHR) {
            	...
            	if(data == 2){
					console.log('預約成功');
					if(sql_status == "insert"){
						alert('預約成功!請在預約到期日之前來店取桌遊，謝謝。');
					}else if(sql_status == "delete"){
						alert('取消預約成功!歡迎您再次在本網站選桌遊，謝謝。');
					}
				}else if(data == 5){
					alert('桌遊都借光光了!下次請早，謝謝。');
				} else {
            		alert('系統錯誤，請通知網站管理員');
            	}
            },
