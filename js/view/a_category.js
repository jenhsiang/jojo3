...
Ext.onReady(function() {
  ...
  //deliver_main
    var columns = [
        {header: '編號', dataIndex: 'category_id', renderer: fGridTooltips, sortable: true},
        {header: '類別名稱', dataIndex: 'category_name', renderer: fGridTooltips, sortable: true},
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        tbar: [{ 
                ...
                store : Ext.create('Ext.data.Store',{ 
                    fields : ['name','value'], 
                    data : [{'name':'類別名稱','value':'category_name'},
    ...
    var form = new Ext.form.FormPanel({
       ...
       /* defaults: {
        ...
        buttons: [{
            ...
            text: '新增',
            handler: function() {
               /* if (form.getForm().findField("category_id").getValue() == "") {
                	alert('請選擇一筆資料');
                } else {*/
                    // 修改
                    ...
                    form.getForm().submit({
                    	timeout : 60,
                    	waitMsg : "正在處理中.....",
                        url: '/controller/a_category_save.jsp',
                        success: function(f, action) {
                            if (action.result.success == 1) {
                                Ext.Msg.alert('消息', action.result.msg, function() {
									grid.getStore().reload();
								  if(action.result.category_id != 0){
										store.reload({ 
								...
								Ext.Msg.alert('消息', action.result.msg, function() {
                                    form.getForm().findField("category_name").setValue("");
                                }); 
							}
                        },
